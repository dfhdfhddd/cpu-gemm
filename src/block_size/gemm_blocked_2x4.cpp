...
// 2x4 微内核：计算 C 的两行、四列，并覆盖 [k_start, k_end) 归约区间。
inline void core2x4_optimized(const float *A, const float *B, float *C,
...
void gemm_blocked_2x4_impl(const float *A, const float *B, float *C, int N, int Mc, int Nc, int Kc){
    for(int i=0;i<N*N;i++){
        C[i] = 0.0f;
    }
    ...
    for(int i=0; i<N; i+=Mc){
        ...
        for(int col=0; col<N; col+=Nc){
            ...
            for(int red=0; red<N; red+=Kc){
                ...
                // 1. 计算能被 2 整除的安全主体边界
                int i_main = i + ((i_end - i) / 2) * 2;
                int col_main = col + ((col_end - col) / 4) * 4;

                // 2. 满血输出区：跑 2x4 微内核
                for (int ii = i; ii < i_main; ii+=2){
                    for (int jj = col; jj < col_main; jj+=4){
                        core2x4_optimized(A, B, C, ii, jj, red, red_end, N);
                    }
                }
...
void gemm_blocked_64x64_2x4(const float* A, const float* B, float* C, int N){
    gemm_blocked_2x4_impl(A, B, C, N, 64, 64, 64);
}

void gemm_blocked_128x128_2x4(const float* A, const float* B, float* C, int N){
    gemm_blocked_2x4_impl(A, B, C, N, 128, 128, 128);
}
